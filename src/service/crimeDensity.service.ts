...
export async function createCrimeDensity(input: { lat: number; lon: number}) {
  ...
  try {
    ...
    const ml_result = await getCrimeDensityPredictions({'latitude':input.lat,'longitude':input.lon,
